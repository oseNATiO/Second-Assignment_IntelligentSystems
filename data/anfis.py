...
from simpful.simpful import FuzzySystem
import copy

        
class ANFIS:

    def __init__(self, X, Y, initParams, conseq_params=None):
        self.X = np.array(copy.copy(X))
        self.Y = np.array(copy.copy(Y))
        self.XLen = len(self.X)
        
        if type(initParams) == membershipfunction.MemFuncs:
            self.memClass = copy.deepcopy(initParams)
        ...
        elif type(initParams) == FuzzySystem:
            var_names = list(initParams._variables.keys())
            num_of_rules = len(initParams.get_rules())
            ling_terms = [initParams.get_fuzzy_sets(var_names[0])[i]._term for i in range(0, num_of_rules)]
            self.memClass = [[] for _ in range(0,len(var_names))]
            for i in range(0, len(var_names)):
                self.memClass[i] = [[] for _ in range(0, num_of_rules)]  
            for i in range(0, len(var_names)):
                for j in range(0, len(ling_terms)):
                    self.memClass[i][j] = ['gaussmf',{'mean':initParams.get_fuzzy_set(var_names[i], ling_terms[j])._funpointer._mu,
                                                      'sigma':initParams.get_fuzzy_set(var_names[i], ling_terms[j])._funpointer._sigma}]     
            self.memClass = membershipfunction.MemFuncs(self.memClass)
            self.memFuncs = self.memClass.MFList        
            self.memFuncsByVariable = [[x for x in range(len(self.memFuncs[z]))] for z in range(len(self.memFuncs))]
            
            self.rules = np.column_stack(self.memFuncsByVariable)

            
            # conseq_params = [initParams._outputfunctions.get(fun).split('+') for fun in initParams._outputfunctions.keys()]
            # self.consequents = np.zeros((len(conseq_params),len(conseq_params[0])))
            # for i in range(0,self.consequents.shape[0]):
            #     for j in range(0, self.consequents.shape[1]-1):
            #         self.consequents[i,j] = np.float(conseq_params[i][j].split("*")[0])
            #     self.consequents[i,j+1] = np.float(conseq_params[i][2])
            # self.consequents = self.consequents.reshape(-1,)
            self.consequents = conseq_params.reshape(-1,1)
